...
fn init_empty() {
    if cfg!(not(miri)) {
        ...
        let vec: SliceVec<usize> = SliceVec::new(token.weak());
...
fn init_capacity() {
    ...
    let mut vec = SliceVec::with_capacity(token.weak(), 10);
...
fn init_empty_push() {
    ...
    let mut vec = SliceVec::new(token.weak());
...
fn drop() {
    ...
    {
        let mut vec = SliceVec::with_capacity(token.weak(), 15);
